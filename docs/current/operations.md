...
<!-- docs-gen:begin id="operations-commands" -->
...
| npm run docs:check | fail when generated docs/current regions drift from their machine sources | package.json (scripts.docs:check) |
| npm run docs:generate | regenerate the machine-owned docs/current content from docs-gen.yaml providers | package.json (scripts.docs:generate), bin/generate-docs.ts |
...
| sdlc <review-stage> --change <name> --reject --failures <file\|-> | record an evidence-backed rejection: the failures YAML names each failed semantic check by its full declared question text copied verbatim (numbers are not names), one entry per check with all findings merged into the single entry's evidence; `-` reads the document from stdin; `--list-semantic-checks` prints the declared checks without opening or writing any round | src/scripts/lib/kinds/review.ts, src/scripts/lib/review-findings.ts |
...
<!-- docs-gen:begin id="operations-environment" -->
...
<!-- docs-gen:end id="operations-environment" -->

## Testing

<!-- docs-gen:begin id="operations-testing" -->
| Test Command | Coverage Tool | Evidence |
| --- | --- | --- |
| npm run test | node --test over unit + e2e | package.json |
| npm run test:all | validate + unit + e2e | package.json |
...
<!-- docs-gen:end id="operations-testing" -->

## Deployment

<!-- docs-gen:begin id="operations-deployment" -->
| Target | Command/Trigger | Evidence |
| --- | --- | --- |
| npm run deploy | deploy both skills + rendered agents to a destination agent root | package.json (scripts), bin/deploy-to-agent.ts |
