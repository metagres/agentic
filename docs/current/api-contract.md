...
| CLI | sdlc <stage-id> --change <change-name> [kind flags] | none | --change + flags per kind (authoring: --request, --finalize, --confirm-semantic, --help-step, --record-answers <file>; review: --accept, --reject, --dry-run, --note, --findings <file>; tasks: --task-id, --status, --note — required for --status done, else TASK_DONE_REQUIRES_NOTE) | frozen envelope | src/scripts/workflows/index.ts, src/scripts/lib/kinds/ | No |
...
- Review round lifecycle (open-to-closed): a bare review invocation opens an inspection round — or refreshes the existing open round in place with its round number frozen — instead of appending a content-free round; a verdict invocation (`--accept`/`--reject`) completes the latest open round in place, appending a closed round only when no open round exists. Rounds carry `status` (open \| closed; legacy rounds without a status field are treated as closed), `rationale` (the `--note` text, else a deterministic summary of the blocking mechanical findings, else omitted), an optional `semantic` block of per-check `{check_id, status, evidence}` results validated against the target stage's semantic-checks.yaml, and reviewer findings `{target, finding, fix?}` — never a severity field. `--note` and `--findings` are verdict-scoped and mutually exclusive; violations are refused with nothing written. Decision tokens: review \| accepted \| rejected \| accept_blocked (accept with blocking findings: the round completes closed, the artifact is untouched, and the envelope is blocked with CANNOT_ACCEPT). Findings-file shape violations refuse the invocation (FINDINGS_ENTRY_INVALID, SEMANTIC_WALK_INVALID); an id-shaped finding target matching no known artifact id only warns (UNKNOWN_FINDING_TARGET) while the round is still recorded — free-text targets never warn. Evidence: src/scripts/lib/kinds/review.ts, src/scripts/lib/review-findings.ts, src/policies/errors.yaml.
