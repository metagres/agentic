...
| Living docs (docs/current/) updated only through knowledge extraction and the docs-gen tooling; authoring stages emit delta entries and never edit docs/current directly; docs-gen owns whole-file generated documents (dependencies, known-issues, decisions) and `<!-- docs-gen -->` marker regions in the mixed documents, enforced fresh by docs:check in the fast gate; deltas targeting generated content are refused (DELTA_TARGETS_GENERATED_REGION) | AGENTS.md §2, src/scripts/lib/kinds/aggregator.ts, bin/generate-docs.ts |
