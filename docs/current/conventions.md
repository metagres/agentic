...
| CLI-only change artifacts: change artifacts under docs/changes/<slug>/ are created and modified only through the sdlc CLI — never by direct agent file edits; the v2 opencode renderer path-scopes every deployed agent's file-write permissions (edit, write, apply_patch) with deny patterns for docs/changes/**, and the generated SKILL.md states the rule (stable marker gated by the deploy smoke); bash-invoked CLI writes are unaffected by the deny | src/scripts/lib/deploy/platforms/opencode.ts, bin/deploy-to-agent.ts |
