...
| YAML key casing: every key in artifacts, stage descriptors, stores, and envelope data is snake_case; CLI flags stay kebab-case, status values stay kebab-case, error codes stay SCREAMING_SNAKE — enumerated schema sections reject unknown keys outright (additionalProperties: false) and free-form sections enforce the casing pattern (propertyNames ^[a-z][a-z0-9_]*$) | all authored artifacts and YAML the toolkit owns | src/stages/{requirements,design,planning}/schema.yaml |
...
| Agent output discipline: chat replies are terse — no preamble, acknowledgments, or self-introduction; no restating the user's request or step instructions; no recapping CLI envelope output; a finished step is reported in one line naming the artifact path; blocked states report what is blocked, why, and what unblocks; review reports are failures-only; implementation notes are one sentence | every agent reply during stage work | src/agents/*.yaml, src/stages/*-review/steps.yaml, src/scripts/lib/kinds/tasks.ts |
...
| docs/current routing map: AGENTS.md §4 (Session Context Routing) is the sole validated map of docs/current — an authoritative selection menu keyed by session goal, with routing rows for docs/changes/<slug>/ and docs/ideas/; the delta-target registry is a docs/current directory scan; index.md is retired | AGENTS.md §4, src/scripts/lib/kinds/aggregator.ts |
...
| Living docs (docs/current/): docs-gen owns whole-file generated documents (dependencies, known-issues, decisions) and `<!-- docs-gen -->` marker regions in the mixed documents, written only by docs:generate and enforced fresh by docs:check in the fast gate (deltas targeting generated content are refused, DELTA_TARGETS_GENERATED_REGION); all other docs/current content is updated directly by the code change that drifted it (AGENTS.md §3 docs-current duty) or by knowledge extraction; SDLC authoring stages still emit delta entries and never edit docs/current directly | AGENTS.md §2, §3, src/scripts/lib/kinds/aggregator.ts, bin/generate-docs.ts |
| CLI-only change artifacts: change artifacts under docs/changes/<slug>/ are created and modified only through the sdlc CLI — never by direct agent file edits; the v2 opencode renderer path-scopes every deployed agent's file-write permissions (edit, write, apply_patch) by stage kind — authoring agents deny base plus .tmp/** scratch allows, the knowledge-curator additionally allows docs/current/**, the implementation agent keeps its allow base, review agents deny — with deny patterns for docs/changes/** always last, and the generated SKILL.md states the rule (stable marker gated by the deploy smoke); bash-invoked CLI writes are unaffected by the deny; concurrent sessions mutating one change are last-write-wins (no CLI locking — documented limitation) | src/scripts/lib/deploy/platforms/opencode.ts, bin/deploy-to-agent.ts |
...
The capped check catalog with each check's required parameters, path-addressed parameter slots, and the stages that declare it (generated from the catalog and the stage structural-checks.yaml files):

<!-- docs-gen:begin id="conventions-checks-reference" -->
| Check | Required params | Path params | Declared by |
| --- | --- | --- | --- |
| all-tasks-terminal | array, allowed_statuses | — | implementation |
| dependency-acyclic | array | — | planning |
| dependency-order | array | — | planning |
| duplicate-refs | array, list_field | — | design |
| forbidden-words | fields | fields (leaf) | requirements |
| given-when-then | arrays | arrays (collection) | requirements |
