...
| Agent output discipline: chat replies are terse — no preamble, acknowledgments, or self-introduction; no restating the user's request or step instructions; no recapping CLI envelope output; a finished step is reported in one line naming the artifact path; blocked states report what is blocked, why, and what unblocks; review reports are failures-only; implementation notes are one sentence | every agent reply during stage work | src/scripts/lib/agent-output-discipline.ts, src/stages/*-review/steps.yaml, src/scripts/lib/kinds/tasks.ts |
| Output-discipline single source: the canonical fragment text rendered at deploy (OUTPUT_DISCIPLINE_FRAGMENT in src/scripts/lib/agent-output-discipline.ts) is the one definition — composed into every AgentRecord.effectivePrompt at registry load and emitted verbatim by both opencode renderer versions; descriptors embedding fragment text fail validation (AGENT_FRAGMENT_COPY); the failures-only review report (each declared-check failure as {check, evidence} with quoted evidence and exact location; never passed checks; zero failures = one line + verdict) is carried verbatim in the four review steps.yaml files and the stage-reviewer prompt, and the one-sentence implementation note (what changed + how verified) in the tasks GUARDRAILS and implementation steps with the write-time guard and required-note-for-status unchanged | every rendered agent; all four review stages; implementation stage | src/scripts/lib/agent-registry.ts, src/scripts/lib/deploy/platforms/opencode.ts, bin/validate-policies.ts, src/agents/stage-reviewer.yaml |
