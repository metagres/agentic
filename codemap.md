...
| `src/skills/` | Version-controlled skill sources: the knowledge-init SKILL.md deployed as a second self-contained skill beside agentic-sdlc, plus `agent-audit/` — a development-only skill (live catalog refresh, web-grounded model assignment, permission auto-fix, roster dedupe/add/remove) that is **never** bundled or deployed. | documented here |
